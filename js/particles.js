...
if (document.body.animate) {
    ...
    document.querySelector('#enemy').addEventListener('click', pop);
  }
  ...
  function createParticle (x, y) {  
    ...
    particle.style.borderRadius = "0px 50% 50% 50%";
    // Generate a random color in a blue/purple palette
    particle.style.background = `hsl(${Math.random() * 1 + 10}, 100%, 50%)`;
    
    // Generate a random x & y destination within a distance of 75px from the mouse
    const destinationX = x + (Math.random() - 0.4) * 2 * 80;
    const destinationY = y + (Math.random() - 0.2) * 2 * 84;
